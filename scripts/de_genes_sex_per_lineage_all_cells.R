...
DE_supporting_lineage_early_prog_sex <- function(
...
early_prog_sex_DE <- DE_supporting_lineage_early_prog_sex(
	rpkm_matrix=all,
...
write.csv(early_prog_sex_DE, file=paste0("../graph/all/", format(Sys.time(), "%Y%m%d"), "_DE_supporting_lineage_early_prog_sex.csv"))
...
prog_genes <- read.csv(file="/media/windows/Users/zazooo/ownCloud/PhD/scRNAseq/Papers/Articles/Ovary_Dev/review_version/SupplementaryData/supplementary_data_6_progenitor_lineage_k-mean_25.csv")
...
join0 <- left_join(genes, prog_genes)
colnames(join0) <- c("genes", "clusters")
join1 <- left_join(join0,early_prog)
colnames(join1) <- c("genes", "clusters", "DE in Early prog", "qvalue1")
