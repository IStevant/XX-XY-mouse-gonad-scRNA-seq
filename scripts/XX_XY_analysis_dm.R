...
# P6 <- names(female_stages[female_stages=="P6"])
# females <- females[,!colnames(females) %in% P6]
# female_count <- female_count[,!colnames(female_count) %in% P6]


female_clustering <- read.csv(file="../data/180712_female_clustering_final.csv", row.names=1)$x
female_clustering <- paste("XX", female_clustering, sep="_")

# female_clustering <- female_clustering[!rownames(female_clustering) %in% P6,]
...
all_sexPalette <- rev(c("#77b0f3", "#fb7072"))
all_captures <- sapply(strsplit(colnames(all), "_"), `[`, 3)

all_data <- log(all+1)


male_clustering <- read.csv(file="../data/170928_male_clustering_final.csv", row.names=1)$x
...
all_clusterPalette <- c(female_clusterPalette, male_clusterPalette)
...
pdf(file=paste0("../graph/all/", format(Sys.time(), "%Y%m%d"), "_all_clustering.pdf"), width=5, height=5)

all_sub_data <- getMostVarGenes(all, fitThr=1.75)
all_sub_data <- log(all_sub_data+1)
all_data <- all_sub_data
...
######## Long to compute, but the result is 19 significant PCs
...
all_t_sne_all_stages <- plot_tSNE_2(
	tsne=all_t_sne, 
	conditions=all_stages, 
	colours= all_stagePalette
)

all_t_sne_all_clusters <- plot_tSNE_2(
	tsne=all_t_sne, 
	conditions=all_clustering, 
	colours= all_clusterPalette
)

all_t_sne_sex <- plot_tSNE_2(
	tsne=all_t_sne, 
	conditions=all_sex, 
...
all_t_sne_new_clusters <- plot_tSNE_2(
	tsne=all_t_sne, 
...
all_clusterPalette2 <- c("#00c5ec", "#eb6bac", "#8a00b0")
...
pdf(file=paste("../graph/all/", format(Sys.time(), "%Y%m%d"), "_progenitors_genes.pdf"), width=10, height=12)
...
pdf(file=paste("../graph/all/", format(Sys.time(), "%Y%m%d"), "_supporting_e12_e13_genes.pdf"), width=10, height=12)
do.call(grid.arrange,c(p, ncol=2))
dev.off()
...
de_sert <- read.csv(file="male_lineage1_gene_var_genes_pseudotime_df_3_new_psdt.csv", row.names=1)
de_grac <- read.csv(file="../graph/female/20180720_female_lineage1_pseudotime_DE_genes.csv", row.names=1)
...
# L1_lineage <- male_pseudotime[,1]
# names(L1_lineage) <- rownames(male_pseudotime)
# L1_lineage <- L1_lineage[!is.na(L1_lineage)]
...
# L2_lineage <- female_pseudotime[,1]
# names(L2_lineage) <- rownames(female_pseudotime)
# L2_lineage <- L2_lineage[!is.na(L2_lineage)]
...
# L3_lineage <- pseudotime[!is.na(pseudotime[,3]),3]
# L3_ordered_lineage <- L3_lineage[order(L3_lineage, decreasing = TRUE)]
# L3_rpkm_exp_lineage <- de_matrix[,names(L3_ordered_lineage)]
# L3_cells <- L3_rpkm_exp_lineage[,order(match(names(L3_ordered_lineage), L3_rpkm_exp_lineage))]



L1_lineage_cells <- names(L1_ordered_lineage)
L2_lineage_cells <- names(L2_ordered_lineage)
# L3_lineage_cells <- names(L3_ordered_lineage)
...
clusters <- cutree(gene_clustering$tree_row, k = 10)
...
# tiff(file=paste0("../graph/all/", format(Sys.time(), "%Y%m%d"), "_supporting_heatmap_DE_genes_granulosa_sertoli_k_15_p-val_0.05_span_0.4.tiff"), res = 300, height = 21, width = 18, units = 'cm')
...
# write.csv(clustering, file=paste0("../graph/all/", format(Sys.time(), "%Y%m%d"), "_supporting_heatmap_DE_genes_granulosa_sertoli_k_15_p-val_0.05.csv"))
